...
class TemporalAggregation(Module):
    def __init__(self,
        ...
        assert embed_dim[0] % num_heads == 0, ValueError(f"Embed_dim must be divisible by num_heads. "
                                                         f"Get '{in_channels}' % {num_heads}")
    ...
    def _compute_hidden_state(self, q: Tensor, k: Tensor, v: Tensor, next_k: Tensor = None) -> Tensor:
        """
        ...
        Algo ref:
            https://towardsdatascience.com/large-language-models-deberta-decoding-enhanced-bert-with-disentangled-attention-90016668db4b
        """
        ...
        if self._relative_pe is not None:
            ...
            c2p_attn: Tensor = q @ rel_k.transpose(-1, -2)
            ...
            p2c_attn: Tensor = k @ rel_q.transpose(-1, -2)
        ...
        c2c_attn: Tensor = q @ k.transpose(-2, -1)  # self-attn
        cross_c2c_attn: Tensor | None = q @ next_k.transpose(-2, -1) if next_k is not None else torch.tensor(0)  # cross-attn

        attn_score: Tensor = c2c_attn + cross_c2c_attn + c2p_attn + p2c_attn
        ...
        if self._num_backbones == 1:
            ...
            for i in range(self._num_backbones):
                ...
                if i == 0:
                    ...
                    hidden_state: Tensor = self._compute_hidden_state(*cache["last_backbone"], current_backbone[1])

                output = hidden_state if output is None else output + hidden_state
                cache["last_backbone"] = current_backbone

            # Last backbone
            hidden_state: Tensor = self._compute_hidden_state(*cache["last_backbone"], cache["first_k"])
            output = hidden_state if output is None else output + hidden_state
