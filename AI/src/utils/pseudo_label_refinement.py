...
class PseudoLabelRefiner(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.__eps = eps
        self.__MA_filter = torch.nn.AvgPool1d(kernel_size, 1, kernel_size // 2)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Refines pseudo-labels using moving average and min-max normalization.

        :param x: anomalous scores for each video segment. Shape [batch_size, num_segments, num_classes=1]
        :return: Refined pseudo-labels. Shape [batch_size, num_segments, num_classes=1]
        """
        x = self.__MA_filter(x)
