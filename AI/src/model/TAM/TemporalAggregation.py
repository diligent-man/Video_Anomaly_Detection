...
class TemporalAggregation(torch.nn.Module):
    """
    Temporal aggregation module from Distilling Aggregated Knowledge
    ...
    def __init__(self,
        ...
        self.__num_backbones: int = num_backbones
        self.__max_rel_pos: int = max_rel_pos

        self.__rel_pos_proj = RelPosProj(embed_dim, max_rel_pos, *args, **kwargs)

        self.__qkv_proj_lst = torch.nn.ModuleList()
        for i in range(num_backbones):
            self.__qkv_proj_lst.append(QKVProj(embed_dim, *args, **kwargs))
    ...
    def _compute_hidden_state(self,
        ...
        rel_q, rel_k = self.__rel_pos_proj(seq_len)
        ...
        attn_score /= (((self.__num_backbones + 2) * self.__embed_dim) ** .5)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        assert num_backbones == self.__num_backbones, "Input backbones does not match"
        assert embed_dim == self.__embed_dim, "Input embed dim does not match"

        output: torch.Tensor | None = None

        if self.__num_backbones == 1:
            x = x.squeeze(dim=0)
            q, k, v = self.__qkv_proj_lst[0](x)  # [batch_size, seq_len, embed_dim]
            output = self._compute_hidden_state(seq_len, q, k, v)
        else:
            hidden_state: None = None
            cache: Dict[str, torch.Tensor | List[torch.Tensor]] = {}

            for i in range(self.__num_backbones):
                q, k, v = self.__qkv_proj_lst[i](x[i])  # [batch_size, seq_len, embed_dim]
                current_backbone = [q, k, v]

                if i == 0:
                    cache["first_k"] = k
                elif i == self.__num_backbones:
